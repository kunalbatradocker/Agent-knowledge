...
/**
 * Generate a short hash fingerprint from sorted column headers.
 * Used to key saved column mappings per document schema shape.
 */
function schemaFingerprint(headers) {
  const crypto = require('crypto');
  const sorted = [...(headers || [])].filter(h => h !== '__sheet').sort().join('|');
  return crypto.createHash('sha256').update(sorted).digest('hex').substring(0, 12);
}

/**
 * Build the Redis key for saved column mappings.
 * Includes schema fingerprint so different CSV structures don't overwrite each other.
 */
function colmapKey(workspaceId, ontologyId, headers) {
  const fp = schemaFingerprint(headers);
  return `colmap:${workspaceId}:${ontologyId}:${fp}`;
}

/**
 * Build the Redis key for column mapping version history.
 */
function colmapHistoryKey(workspaceId, ontologyId, headers) {
  const fp = schemaFingerprint(headers);
  return `colmap_history:${workspaceId}:${ontologyId}:${fp}`;
}
 ...
 * List staged documents for the current workspace — workspace-scoped.
 * MUST be before /:id route.
 */
router.get('/staged', optionalTenantContext, async (req, res) => {
  try {
    const redisService = require('../../services/redisService');
    ...
    const jobs = await ontologyJobService.getJobs(jobFilters);
    ...
    for (const job of stagedJobs) {
      ...
      if (stagedJson) {
        ...
        // Double-check workspace match (staged doc stores workspace_id inside)
        const docWs = staged.document?.workspace_id;
        if (workspaceId && docWs && docWs !== workspaceId) continue;
    ...
    // Also scan Redis for orphaned staged docs (no job) — filter by workspace
    try {
 ...
 * Get specific staged document — verifies workspace access
 */
router.get('/staged/:docId', optionalTenantContext, async (req, res) => {
  try {
    ...
    const docWs = staged.document?.workspace_id;
    if (workspaceId && docWs && docWs !== workspaceId) {
      return res.status(404).json({ success: false, error: 'Staged document not found' });
    }
 ...
 * Delete a staged document — verifies workspace access
 */
router.delete('/staged/:docId', optionalTenantContext, async (req, res) => {
  try {
    const { docId } = req.params;
    const workspaceId = req.query.workspace_id || req.tenantContext?.workspace_id || req.headers['x-workspace-id'];
    const redisService = require('../../services/redisService');

    // Verify workspace access before deleting
    const stagedJson = await redisService.get(`staged:${docId}`);
    if (stagedJson) {
      const staged = JSON.parse(stagedJson);
      const docWs = staged.document?.workspace_id;
...
router.get('/column-mappings', async (req, res) => {
  try {
    const { ontologyId, workspaceId, headers: headersParam } = req.query;
    if (!ontologyId || !workspaceId) {
      return res.status(400).json({ success: false, error: 'ontologyId and workspaceId are required' });
    }
    
    // Try schema-fingerprinted key first (new format), fall back to legacy key
    const sourceHeaders = headersParam ? JSON.parse(headersParam) : null;
    let key, historyKey;
    let saved = null;
    
    if (sourceHeaders && sourceHeaders.length > 0) {
      key = colmapKey(workspaceId, ontologyId, sourceHeaders);
      historyKey = colmapHistoryKey(workspaceId, ontologyId, sourceHeaders);
      saved = await redisService.get(key);
    }
    
    // Fall back to legacy key (without fingerprint) for backward compat
    if (!saved) {
      const legacyKey = `colmap:${workspaceId}:${ontologyId}`;
      saved = await redisService.get(legacyKey);
      if (saved) {
        historyKey = `colmap_history:${workspaceId}:${ontologyId}`;
      }
    }
    ...
    // Load version history (historyKey already set above)
    let versions = [];
    try {
      if (historyKey) {
...
router.post('/column-mappings', async (req, res) => {
  try {
    ...
    const effectiveHeaders = sourceHeaders || Object.keys(columnMappings);
    const key = colmapKey(workspaceId, ontologyId, effectiveHeaders);
    const hKey = colmapHistoryKey(workspaceId, ontologyId, effectiveHeaders);
    ...
    if (existing) {
      let history = [];
      try {
        const historyJson = await redisService.get(hKey);
      ...
      await redisService.set(hKey, JSON.stringify(history), 0);
    }
    ...
    if (docJson) {
      ...
      try {
        ...
        if (cachedFilters) {
          ...
          // Get all unique entity types for filter dropdown (unfiltered) with counts
          const typesQuery = `
            SELECT ?type (COUNT(DISTINCT ?entity) AS ?cnt) WHERE {
          ...
          allEntityTypes = (typesResult?.results?.bindings || []).map(b => {
            ...
            const label = val.includes('#') ? val.split('#').pop() : val.split('/').pop();
            const count = parseInt(b.cnt?.value || 0);
            return { label, count };
          }).filter(b => b.label);

          // Get all unique predicates for filter dropdown (unfiltered) with counts
          const predsQuery = `
            SELECT ?p (COUNT(*) AS ?cnt) WHERE {
              ...
              FILTER(isIRI(?o))
              FILTER(?p != <http://purplefabric.ai/ontology#sourceDocument>)
              FILTER(?p != <http://purplefabric.ai/ontology#lastUpdatedBy>)
              FILTER(?p != <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>)
        ...
        // Only require ?s to have sourceDocument; ?o just needs to be a URI
        // (stub linked entities like DeviceType may not have sourceDocument)
        const relCountQuery = `
      ...
      if (allEntityTypes.length === 0 && entities.length > 0) {
        const typeCounts = {};
        entities.forEach(e => { if (e.type) typeCounts[e.type] = (typeCounts[e.type] || 0) + 1; });
        allEntityTypes = Object.entries(typeCounts).sort(([a],[b]) => a.localeCompare(b)).map(([label, count]) => ({ label, count }));
      ...
      if (allPredicates.length === 0 && relations.length > 0) {
        const predCounts = {};
        relations.forEach(r => { if (r.predicate) predCounts[r.predicate] = (predCounts[r.predicate] || 0) + 1; });
        allPredicates = Object.entries(predCounts).sort(([a],[b]) => a.localeCompare(b)).map(([label, count]) => ({ label, count }));
      ...
      return res.json({
        ...
        document: {
          ...
          source_headers: docMeta.source_headers ? (typeof docMeta.source_headers === 'string' ? JSON.parse(docMeta.source_headers) : docMeta.source_headers) : null
        },
        chunks: chunks.map(c => ({
          chunk_id: c.id,
          uri: c.id,
...
router.delete('/:id', optionalTenantContext, async (req, res) => {
  try {
    ...
    const session = neo4jService.getSession();
    const results = { document: false, chunks: 0, concepts: 0, entities: 0, redis: false, graphdb: false };
    
    try {
      // Resolve document URI — prefer Redis metadata, fall back to Neo4j
      let docUri = docMeta?.uri || null;
      if (!docUri) {
        const docResult = await session.run(`
          MATCH (d:Document) WHERE d.doc_id = $docId OR d.uri = $docId
          RETURN d.uri as uri
        `, { docId });
        docUri = docResult.records[0]?.get('uri');
      }
      if (!docUri) {
        docUri = `doc://${docId}`;
      }
      ...
      // Delete entity nodes synced from this document's triples in Neo4j
      // sourceDocument is stored as a PROPERTY (not a relationship) by the sync service
      if (docUri) {
        const entityResult = await session.run(`
          MATCH (e)
          WHERE e.sourceDocument = $docUri
          DETACH DELETE e RETURN count(e) as deleted
        `, { docUri });
        results.entities = neo4jService.toNumber(entityResult.records[0]?.get('deleted') || 0);
        logger.info(`[delete] Removed ${results.entities} entity nodes from Neo4j for doc ${docId} (uri: ${docUri})`);
      }
      ...
      // Delete from GraphDB — use separate queries to avoid UNION issues
      if (docUri) {
        try {
          const graphDBStore = require('../../services/graphDBStore');
          ...
          // 1. Find all entity URIs sourced from this document
          const findQuery = `
          ...
          `;
          const findResp = await graphDBStore._fetchWithPool(
            `${graphDBStore.baseUrl}/repositories/${graphDBStore.repository}?query=${encodeURIComponent(findQuery)}`,
          ...
          if (findResp.ok) {
            ...
            entityURIs = (findData.results?.bindings || []).map(b => b.s?.value).filter(Boolean);
            logger.info(`[delete] Found ${entityURIs.length} entities sourced from doc ${docUri}`);
          }

          // 2. Delete all triples where these entities are SUBJECT
          const deleteEntitiesQuery = `
            DELETE {
              GRAPH <${dataGraphIRI}> { ?s ?p ?o }
            } WHERE {
              GRAPH <${dataGraphIRI}> {
                ?s <http://purplefabric.ai/ontology#sourceDocument> <${docUri}> .
                ?s ?p ?o .
          ...
          `;
          const resp1 = await graphDBStore._fetchWithPool(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/sparql-update' },
            body: deleteEntitiesQuery
          });
          if (!resp1.ok) {
            const err = await resp1.text();
            logger.warn(`[delete] GraphDB entity triple delete failed: ${resp1.status} - ${err}`);
          } else {
            logger.info(`[delete] Removed entity triples (as subject) from GraphDB for doc: ${docUri}`);
          }
          
          // 3. Delete triples where these entities are OBJECT (incoming relationships from other entities)
          if (entityURIs.length > 0) {
            const BATCH = 200;
            for (let i = 0; i < entityURIs.length; i += BATCH) {
              const batch = entityURIs.slice(i, i + BATCH);
              const valuesClause = batch.map(u => `<${u}>`).join(' ');
              const deleteIncomingQuery = `
                DELETE {
                  GRAPH <${dataGraphIRI}> { ?s ?p ?o }
                } WHERE {
                  GRAPH <${dataGraphIRI}> {
                    ?s ?p ?o .
                    VALUES ?o { ${valuesClause} }
                  }
                }
              `;
              const respInc = await graphDBStore._fetchWithPool(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/sparql-update' },
                body: deleteIncomingQuery
              });
              if (!respInc.ok) {
                logger.warn(`[delete] GraphDB incoming relationship delete batch ${Math.floor(i/BATCH)+1} failed`);
              }
            }
            logger.info(`[delete] Removed incoming relationship triples for ${entityURIs.length} entities`);
          }
          
          // 4. Delete triples where doc is subject (document node itself)
          const deleteDocTriplesQuery = `
            DELETE {
              GRAPH <${dataGraphIRI}> { <${docUri}> ?p ?o }
            } WHERE {
              GRAPH <${dataGraphIRI}> { <${docUri}> ?p ?o }
            }
          `;
          const resp2 = await graphDBStore._fetchWithPool(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/sparql-update' },
            body: deleteDocTriplesQuery
          });
          if (!resp2.ok) {
            const err = await resp2.text();
            logger.warn(`[delete] GraphDB doc triple delete failed: ${resp2.status} - ${err}`);
          }
          
          // 5. Delete partOf triples (chunks referencing this document)
          const deletePartOfQuery = `
            DELETE {
              GRAPH <${dataGraphIRI}> { ?s ?p ?o }
            } WHERE {
              GRAPH <${dataGraphIRI}> {
                ?s <http://purplefabric.ai/ontology#partOf> <${docUri}> .
                ?s ?p ?o .
              }
            }
          `;
          const resp3 = await graphDBStore._fetchWithPool(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/sparql-update' },
            body: deletePartOfQuery
          });
          if (!resp3.ok) {
            const err = await resp3.text();
            logger.warn(`[delete] GraphDB partOf triple delete failed: ${resp3.status} - ${err}`);
          }
          
          results.graphdb = resp1.ok && resp2.ok && resp3.ok;
          results.graphdbEntities = entityURIs.length;
          if (results.graphdb) {
            logger.info(`[delete] Fully removed document from GraphDB: ${docUri} (${entityURIs.length} entities)`);
          }
        } catch (e) {
      ...
      try {
        ...
        // Clean up additional Redis keys
        await redisService.del(`embed_pending:${docId}`);
        await redisService.del(`staged:${docId}`);
        await redisService.del(`rawfile:${docId}`);
        await redisService.del(`cache:filters:${docId}`);
        results.redis = true;
      } catch (e) {
...
/**
 * POST /api/ontology/documents/:id/retry-embeddings
 * Retry failed embeddings for a committed document
 */
router.post('/:id/retry-embeddings', optionalTenantContext, async (req, res) => {
  try {
    const docId = req.params.id;
    const tenantId = req.body.tenantId || req.headers['x-tenant-id'] || 'default';
    const workspaceId = req.body.workspaceId || req.headers['x-workspace-id'] || 'default';
    ...
    const pendingJson = await redisService.get(`embed_pending:${docId}`);
    if (!pendingJson) {
      return res.json({ success: true, message: 'No pending embeddings to retry', retried: 0 });
    }

    const pending = JSON.parse(pendingJson);
    const { chunks, failedIndices, docTitle, docType } = pending;

    if (!failedIndices || failedIndices.length === 0) {
      await redisService.del(`embed_pending:${docId}`);
      return res.json({ success: true, message: 'No failed chunks to retry', retried: 0 });
    }

    // Return immediately, process in background
    res.json({ success: true, message: `Retrying ${failedIndices.length} embeddings in background`, total: failedIndices.length });

    // Background retry with user's Bedrock token
    ...
    try {
      ...
      if (userToken) bedrockToken = userToken;
    } catch (e) { /* use server default */ }

    const { llmRequestContext } = require('../../services/llmService');
    const ctx = bedrockToken ? { bedrockToken } : {};
    llmRequestContext.run(ctx, async () => {
      ...
      for (let b = 0; b < failedIndices.length; b += EMBED_PARALLEL) {
        const batchIndices = failedIndices.slice(b, b + EMBED_PARALLEL);
        const batchChunks = batchIndices.map(idx => chunks[idx]).filter(Boolean);
        const embedResults = await Promise.allSettled(batchChunks.map(c => embeddingService.generateEmbedding(c.text)));

        await Promise.all(batchChunks.map((chunk, j) => {
          ...
          return vectorStoreService.storeChunk({
            id: chunk.uri || `${docId}_chunk_${origIdx}`,
            text: chunk.text,
            documentId: docId,
            ...
            stillFailed.push(origIdx);
            logger.warn(`[retry-embed] Failed chunk ${origIdx}: ${e.message}`);
          });
        }));
      }

      if (stillFailed.length > 0) {
        // Update pending with remaining failures
        await redisService.set(`embed_pending:${docId}`, JSON.stringify({
          ...pending, failedIndices: stillFailed
        }), 7 * 24 * 3600);
        logger.info(`[retry-embed] ${stored} succeeded, ${stillFailed.length} still pending for doc ${docId}`);
      } else {
        // All done — clean up
        await redisService.del(`embed_pending:${docId}`);
        logger.info(`[retry-embed] All ${stored} embeddings completed for doc ${docId}`);
      }

      // Update doc metadata with new chunk count
      try {
        const docJson = await redisService.get(`doc:${docId}`);
        if (docJson) {
          const docMeta = JSON.parse(docJson);
          docMeta.chunks_stored = (docMeta.chunks_stored || 0) + stored;
          docMeta.embedding_failures = stillFailed.length;
          await redisService.set(`doc:${docId}`, JSON.stringify(docMeta), 0);
        }
      } catch (e) { /* non-fatal */ }
      ...
      try {
        const jobs = await ontologyJobService.getJobs({ limit: 100 });
        const job = jobs.find(j => j.staged_doc_id === docId || j.file_name === docTitle);
        if (job) {
          const updates = { embedding_failures: stillFailed.length };
          if (stillFailed.length === 0) {
            updates.progress_message = job.progress_message?.replace(/\s*\(⚠️.*?\)/, '');
          } else {
            updates.progress_message = job.progress_message?.replace(/\(⚠️.*?\)/, `(⚠️ ${stillFailed.length} chunks still need embedding)`);
          }
          await ontologyJobService.updateJob(job.job_id, updates);
        }
      } catch (e) { /* non-fatal */ }
    }).catch(err => logger.error(`[retry-embed] Background retry failed: ${err.message}`));

  } catch (error) {
    logger.error('Retry embeddings error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * POST /api/ontology/documents/:id/re-embed
 * Re-chunk and re-embed from the raw uploaded file stored in Redis.
 * Independent of GraphDB — re-parses the original file from scratch.
 */
router.post('/:id/re-embed', optionalTenantContext, async (req, res) => {
  try {
    const docId = req.params.id;
    const tenantId = req.body.tenantId || req.headers['x-tenant-id'] || 'default';
    const workspaceId = req.body.workspaceId || req.headers['x-workspace-id'] || 'default';
    ...
    // 1. Check if embed_pending exists (prefer retry over full re-embed)
    const pendingJson = await redisService.get(`embed_pending:${docId}`);
    if (pendingJson) {
      return res.json({ success: true, message: 'Found pending embeddings — use retry-embeddings endpoint instead', usePending: true });
    }

    // 2. Load document metadata
    const docJson = await redisService.get(`doc:${docId}`);
    if (!docJson) {
      return res.status(404).json({ success: false, error: 'Document not found in Redis' });
    }
    const docMeta = JSON.parse(docJson);
    const docUri = docMeta.uri;
    const docTitle = docMeta.title || 'Unknown';

    // 3. Load raw file from Redis
    const rawJson = await redisService.get(`rawfile:${docId}`);
    if (!rawJson) {
      return res.status(404).json({ success: false, error: 'Raw file not found in Redis. This document was uploaded before raw file storage was enabled — please re-upload.' });
    }
    const rawMeta = JSON.parse(rawJson);
    const fileBuffer = Buffer.from(rawMeta.base64, 'base64');
    const ext = rawMeta.ext;

    // 4. Write temp file, parse, and chunk
    const os = require('os');
    const tmpPath = path.join(os.tmpdir(), `reembed_${docId}${ext}`);
    fs.writeFileSync(tmpPath, fileBuffer);

    let chunks = [];
    try {
      if (ext === '.csv' || ext === '.xlsx' || ext === '.xls') {
        // Tabular: parse and chunk by rows
        let csvData;
        if (ext === '.csv') {
          csvData = await csvParser.parse(tmpPath, { hasHeader: true });
        } else {
          const excelParser = require('../../services/excelParser');
          const parsed = await excelParser.parse(tmpPath);
          const flat = excelParser.flattenSheets(parsed);
          csvData = {
            headers: flat.headers, rows: flat.rows, rowCount: flat.rowCount,
            sheets: parsed.sheets.map(s => ({ name: s.sheetName, headers: s.headers, rowCount: s.rowCount, rows: s.rows }))
          };
        }

        const rowsPerChunk = 50;
        const hasSheets = csvData.sheets && csvData.sheets.length > 1;

        if (hasSheets) {
          for (const sheet of csvData.sheets) {
            const sheetRows = sheet.rows || [];
            const sheetLabel = sheet.name || sheet.sheetName;
            for (let i = 0; i < sheetRows.length; i += rowsPerChunk) {
              const chunkRows = sheetRows.slice(i, i + rowsPerChunk);
              const chunkText = `[Sheet: ${sheetLabel}]\n` + chunkRows.map(row =>
                Object.entries(row).filter(([k, v]) => v != null && v !== '').map(([k, v]) => `${k}: ${v}`).join(', ')
              ).join('\n');
              chunks.push({ text: chunkText, uri: `${docUri}#sheet=${encodeURIComponent(sheetLabel)}&chunk=${chunks.length}` });
            }
          }
        } else {
          ...
          for (let i = 0; i < rows.length; i += rowsPerChunk) {
            ...
            chunks.push({ text: chunkText, uri: `${docUri}#chunk=${chunks.length}` });
          }
        }
      } else if (ext === '.pdf') {
        // PDF: extract text and chunk
        const pdfData = await pdfParser.extractText(tmpPath);
        const result = chunkingService.chunkDocumentWithMethod(pdfData.text, {
          id: docId, uri: docUri, name: rawMeta.fileName,
          doc_type: 'pdf',
          chunkingMethod: rawMeta.chunkingMethod || 'page',
          numPages: pdfData.numPages || 1,
          pageBreaks: pdfData.pageBreaks || [],
          pageTexts: pdfData.pageTexts || []
        });
        for (const c of result.chunks) {
          chunks.push({ text: c.text, uri: c.uri || `${docUri}#chunk=${chunks.length}` });
        }
      } else {
        // Plain text files
        const text = fs.readFileSync(tmpPath, 'utf-8');
        const result = chunkingService.chunkDocumentWithMethod(text, {
          id: docId, uri: docUri, name: rawMeta.fileName,
          doc_type: ext.replace('.', ''),
          chunkingMethod: rawMeta.chunkingMethod || 'fixed'
        });
        for (const c of result.chunks) {
          chunks.push({ text: c.text, uri: c.uri || `${docUri}#chunk=${chunks.length}` });
        }
    ...
    if (chunks.length === 0) {
      return res.json({ success: true, message: 'No content to chunk from raw file', total: 0 });
    }

    // Return immediately, process embedding in background
    res.json({ success: true, message: `Re-embedding ${chunks.length} chunks from raw file in background`, total: chunks.length });

    // 5. Background: delete old chunks, embed new ones
    const { llmRequestContext } = require('../../services/llmService');
    let bedrockToken = null;
    try {
      const tokenStore = require('../../utils/tokenEncryption');
    ...
    llmRequestContext.run(ctx, async () => {
      ...
      try { await vectorStoreService.deleteDocument(docId); } catch (e) { /* may not exist */ }

      let stored = 0;
      const failedIndices = [];
      const EMBED_PARALLEL = parseInt(process.env.EMBEDDING_PARALLELISM) || 10;

      for (let b = 0; b < chunks.length; b += EMBED_PARALLEL) {
        const batch = chunks.slice(b, b + EMBED_PARALLEL);
        ...
        await Promise.all(batch.map((chunk, j) => {
          if (embedResults[j].status === 'rejected') {
            if (failedIndices.length === 0) {
              logger.warn(`[re-embed] First embedding failure: ${embedResults[j].reason?.message || embedResults[j].reason}`);
            }
            failedIndices.push(b + j);
            return Promise.resolve();
          }
          return vectorStoreService.storeChunk({
            id: chunk.uri || `${docId}_chunk_${b + j}`,
            text: chunk.text,
            documentId: docId,
            documentName: docTitle,
            chunkIndex: b + j,
            tenant_id: tenantId,
            workspace_id: workspaceId,
            doc_type: docMeta.doc_type || ext.replace('.', '')
          }, embedResults[j].value).then(() => { stored++; }).catch(e => {
            failedIndices.push(b + j);
          });
        }));
      ...
      // Save failed chunks for retry if any
      if (failedIndices.length > 0) {
        await redisService.set(`embed_pending:${docId}`, JSON.stringify({
          chunks, failedIndices, tenantId, workspaceId,
          docTitle, docType: docMeta.doc_type || ext.replace('.', '')
        }), 7 * 24 * 3600);
      }

      // Update doc metadata
      try {
        docMeta.chunks_stored = stored;
        docMeta.embedding_failures = failedIndices.length;
        await redisService.set(`doc:${docId}`, JSON.stringify(docMeta), 0);
      } catch (e) { /* non-fatal */ }
      ...
      try {
        ...
        const job = jobs.find(j => j.staged_doc_id === docId || j.file_name === docTitle);
        if (job) {
          const updates = { embedding_failures: failedIndices.length };
          if (failedIndices.length === 0) {
            updates.progress_message = `✅ Committed — ${stored} chunks embedded`;
          } else {
            updates.progress_message = `✅ Committed (⚠️ ${failedIndices.length} chunks still need embedding)`;
          }
          await ontologyJobService.updateJob(job.job_id, updates);
        }
      } catch (e) { /* non-fatal */ }

      logger.info(`[re-embed] Done: ${stored}/${chunks.length} chunks embedded for doc ${docId}${failedIndices.length > 0 ? ` (${failedIndices.length} failed)` : ''}`);
    }).catch(err => logger.error(`[re-embed] Background failed: ${err.message}`));

  } catch (error) {
    logger.error('Re-embed error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.post('/upload-document', upload.single('file'), optionalTenantContext, async (req, res) => {
  try {
    ...
    // Folder is required — files must be organized into folders
    if (!folderId) {
      // Clean up uploaded temp file
      try { require('fs').unlinkSync(filePath); } catch (e) { /* ignore */ }
      return res.status(400).json({ success: false, error: 'folder_id is required. Please select a folder before uploading.' });
    }
  ...
  setImmediate(async () => {
    try {
      ...
      if ((ext === '.csv' || ext === '.xlsx' || ext === '.xls') && csvData) {
        ...
        // Chunk CSV rows for RAG vector search — sheet-aware for multi-sheet workbooks
        let csvChunks = null;
        if (csvData.rows?.length > 0) {
          ...
          if (hasSheets) {
            // Multi-sheet: chunk per-sheet so chunks never mix rows from different sheets
            for (const sheet of csvData.sheets) {
              const sheetRows = sheet.rows || [];
              const sheetLabel = sheet.name || sheet.sheetName;
              for (let i = 0; i < sheetRows.length; i += rowsPerChunk) {
                ...
                csvChunks.push({
                  uri: `${docUri}#sheet=${encodeURIComponent(sheetLabel)}&chunk=${csvChunks.length}`,
                  chunk_id: uuidv4(),
                  text: chunkText,
                  order: csvChunks.length,
                  sheetName: sheetLabel,
                  startRow: i,
                  endRow: Math.min(i + rowsPerChunk, sheetRows.length),
            ...
            logger.info(`Sheet-aware chunking: created ${csvChunks.length} chunks across ${csvData.sheets.length} sheets`);
          } else {
            // Single sheet / CSV: chunk the flat rows array as before
            ...
            logger.info(`CSV chunking: created ${csvChunks.length} chunks from ${csvData.rows.length} rows`);
          }
        }
        ...
        stagedData = {
          ...
          csvData: {
            ...
            rows: csvData.rows,
            sheets: csvData.sheets ? csvData.sheets.map(s => ({ name: s.name || s.sheetName, headers: s.headers, rowCount: s.rowCount })) : null,
            summary: csvSummary
          },
          chunks: csvChunks,
      ...
      // Store raw uploaded file in Redis for re-chunking / re-embedding later
      // Base64-encoded binary — persists until document is deleted
      {
        const redisService = require('../../services/redisService');
        try {
          ...
          await redisService.set(`rawfile:${docId}`, rawPayload, 0); // no TTL
          logger.info(`📦 Stored raw file in Redis for ${docId} (${(fileBuffer.length / 1024).toFixed(1)} KB)`);
        } catch (e) {
          logger.warn(`⚠️ Could not store raw file in Redis for ${docId}: ${e.message}`);
        }
      }
  ...
  try {
    ...
    if (!job) {
      ...
      job = await ontologyJobService.createJob({
        jobType: 'commit',
        fileName: staged.document?.title || 'Document',
        staged_doc_id: docId,
        tenantId,
        workspaceId
      });
    ...
    // Reset processing steps to commit-type steps (upload job may have upload-type steps)
    ...
    await ontologyJobService.updateJob(job.job_id, {
      ...
      processing_steps: JSON.stringify(commitSteps)
    });
...
/**
 * POST /api/ontology/documents/rollback-commit
 * Rollback a committed document by deleting all triples sourced from it.
 * Uses pf:sourceDocument provenance to identify and remove triples.
 */
router.post('/rollback-commit', async (req, res) => {
  try {
    ...
    if (!docUri) {
      return res.status(400).json({ success: false, error: 'Document has no URI — cannot rollback' });
    }
    ...
    const dataGraphIRI = graphDBStore.getDataGraphIRI(tenantId, workspaceId);
    const statementsUrl = `${graphDBStore.baseUrl}/repositories/${graphDBStore.repository}/statements`;

    // Step 1: Find all entity URIs sourced from this document
    const selectUrl = `${graphDBStore.baseUrl}/repositories/${graphDBStore.repository}`;
    const findQuery = `SELECT DISTINCT ?s WHERE { GRAPH <${dataGraphIRI}> { ?s <${PF}sourceDocument> <${docUri}> . } }`;
    const findResp = await graphDBStore._fetchWithPool(`${selectUrl}?query=${encodeURIComponent(findQuery)}`, {
      method: 'GET',
    ...
    if (findResp.ok) {
      const findData = await findResp.json();
      const entityURIs = (findData.results?.bindings || []).map(b => b.s?.value).filter(Boolean);
      entityCount = entityURIs.length;

      if (entityURIs.length > 0) {
        // Step 2: Delete all triples for these entities
        await graphDBTripleService.deleteEntityTriples(tenantId, workspaceId, entityURIs);
      }
    }

    // Step 3: Delete the document node itself
    const deleteDocQuery = `DELETE WHERE { GRAPH <${dataGraphIRI}> { <${docUri}> ?p ?o . } }`;
    await graphDBStore._fetchWithPool(statementsUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/sparql-update' },
      body: deleteDocQuery
    });

    // Step 4: Delete partOf triples (chunks referencing this document)
    const deletePartOfQuery = `DELETE WHERE { GRAPH <${dataGraphIRI}> { ?chunk <http://purplefabric.ai/ontology#partOf> <${docUri}> . ?chunk ?p ?o . } }`;
    await graphDBStore._fetchWithPool(statementsUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/sparql-update' },
      body: deletePartOfQuery
    ...
    // Step 5: Update document metadata to reflect rollback
    docMeta.rolledBack = true;
    docMeta.rolledBackAt = new Date().toISOString();
    await redisService.set(`doc:${docId}`, JSON.stringify(docMeta), 0);

    logger.info(`[rollback] Rolled back doc ${docId} (${docUri}): removed ${entityCount} entities from GraphDB`);

    res.json({
      success: true,
      message: `Rolled back ${entityCount} entities from document "${docMeta.title || docId}"`,
      entityCount,
      docUri
    });
  } catch (error) {
...
async function processCommitInBackground(jobId, docId, staged, options) {
  ...
  // Look up per-user Bedrock token and run entire commit inside llmRequestContext
  // so both LLM calls AND embedding calls can access the bearer token.
  // Without this, BullMQ workers have no HTTP request context and embeddings fail with 403.
  let bedrockToken = null;
  try {
    const tokenStore = require('../../utils/tokenEncryption');
  ...
  const { llmRequestContext } = require('../../services/llmService');
  const ctx = bedrockToken ? { bedrockToken } : {};
  return llmRequestContext.run(ctx, () => _processCommitInner(jobId, docId, staged, options));
}

async function _processCommitInner(jobId, docId, staged, options) {
  const { ontologyId, columnMappings, primaryClass, tenantId, workspaceId, selectedSheets, sheetClassMap, extractedRelationships } = options;
  const redisService = require('../../services/redisService');
  ...
  try {
    ...
    await ontologyJobService.updateProcessingStep(jobId, 'extraction', 'active');
    await ontologyJobService.updateJob(jobId, {
      progress: 30,
    ...
    if (isTabularType(staged.type) && staged.csvData) {
      ...
      // Validate that mapping keys match actual CSV headers (Fix #3)
      // For multi-sheet workbooks, mapping keys use "SheetName:col" format
      const csvHeaders = new Set((staged.csvData.headers || []).filter(h => h !== '__sheet'));
      const hasPerSheetKeys = Object.keys(effectiveMappings).some(k => k.includes(':'));
      const mappingKeys = Object.keys(effectiveMappings).filter(k => !effectiveMappings[k]?.ignore);
      let staleKeys;
      if (hasPerSheetKeys) {
        // Per-sheet keys: "SheetName:col" — validate the column part after the colon
        staleKeys = mappingKeys.filter(k => {
          const colPart = k.includes(':') ? k.split(':').slice(1).join(':') : k;
          return !csvHeaders.has(colPart) && !csvHeaders.has(k);
        });
      } else {
        staleKeys = mappingKeys.filter(k => !csvHeaders.has(k));
      }
      if (staleKeys.length > 0) {
        logger.warn(`[commit-staged] Stale mapping keys not in CSV headers (removed): ${staleKeys.join(', ')}`);
        staleKeys.forEach(k => delete effectiveMappings[k]);
      }
      
      (staged.csvData.headers || []).forEach(col => {
        if (!effectiveMappings[col]) {
          effectiveMappings[col] = { property: '', linkedClass: '', domain: '', domainLabel: '', ignore: false };
        }
      });
...
Return JSON:
{
  ...
  "relationships": [{"type": "REL_TYPE", "from_entity": "name", "to_entity": "name", "confidence": 0.8}]
    ...
    await ontologyJobService.updateProcessingStep(jobId, 'extraction', 'completed');
    await ontologyJobService.updateProcessingStep(jobId, 'writing', 'active');
    await ontologyJobService.updateJob(jobId, {
      progress: 50,
    ...
    await ontologyJobService.updateProcessingStep(jobId, 'writing', 'completed');
    ...
    if (isTabularType(staged.type) && columnMappings && ontologyId) {
      try {
        const commitHeaders = staged.csvData?.headers?.filter(h => h !== '__sheet') || Object.keys(columnMappings);
        const mapKey = colmapKey(workspaceId, ontologyId, commitHeaders);
        const mapHistKey = colmapHistoryKey(workspaceId, ontologyId, commitHeaders);
        ...
        if (existing) {
          ...
          let history = [];
          try {
            const historyJson = await redisService.get(mapHistKey);
            if (historyJson) history = JSON.parse(historyJson);
          } catch (e) { /* no history */ }
          history.unshift({
            version: prev.version || 1,
          ...
          if (history.length > 20) history = history.slice(0, 20);
          await redisService.set(mapHistKey, JSON.stringify(history), 0);
        }
    ...
    const embeddingFailures = result.embeddingFailures || 0;
    const embedMsg = embeddingFailures > 0 ? ` (⚠️ ${embeddingFailures} chunks need embedding retry)` : '';
