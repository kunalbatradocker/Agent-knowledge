...
class OntologyVersioningService {
  ...
  async createVersion(ontologyId, options = {}) {
    ...
    // Get current ontology data from GraphDB — this is the authoritative source
    let ontologyData;
    try {
      ontologyData = await owlOntologyService.getOntologyStructure(tenant_id, workspace_id, ontologyId, 'all');
    } catch (error) {
      console.error(`Could not get ontology structure for ${ontologyId}:`, error.message);
      throw new Error(`Cannot create version: ontology structure not found for ${ontologyId}`);
    }

    if (!ontologyData || (!ontologyData.classes?.length && !ontologyData.properties?.length)) {
      console.warn(`Ontology ${ontologyId} has no classes or properties — creating version with empty structure`);
      ontologyData = ontologyData || { classes: [], properties: [] };
    }
    ...
    // Detect if this is the first version for this ontology
    const isFirstVersion = !actualParent;
    ...
    const versionMeta = {
      ...
      created_by: user_id,
      description: description || (isFirstVersion ? 'Initial version' : ''),
      branch,
      parent_version: actualParent,
      is_initial: isFirstVersion,
    ...
    // Update branch pointer — always ensure main branch exists
    await redisClient.hSet(`${REDIS_KEYS.BRANCHES}${ontologyId}`, branch, versionId);

    // Auto-tag the first version as 'initial' so there's always a baseline to compare against
    if (isFirstVersion) {
      try {
        await this.createTag(ontologyId, 'initial', versionId, {
          user_id,
          description: 'Automatically tagged as the initial baseline version'
        });
      } catch (tagError) {
        // Tag may already exist if ontology was re-imported — non-fatal
        console.warn(`Could not auto-tag initial version: ${tagError.message}`);
      }
    }

    // Create explicit tag if specified
    if (tag && tag !== 'initial') {
      await this.createTag(ontologyId, tag, versionId, { user_id });
    }
    ...
    await this.logAudit(ontologyId, {
      ...
      version_id: versionId,
      user_id,
      details: { description: versionMeta.description, branch, tag, parent_version: actualParent, is_initial: isFirstVersion }
    });
