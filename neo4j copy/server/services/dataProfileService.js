...
class DataProfileService {
  ...
  _detectType(header, values, cardinality, cardinalityRatio, totalRows) {
    ...
    const isIdByName = /^(id|_id|pk|key|uuid|guid)$/i.test(header) ||
      /(_id|_pk|_key|_uuid|Id|ID|Pk|PK)$/i.test(header) ||
      /^(id_|pk_|key_)/i.test(header) ||
      /^(id|pk|key|uuid|guid)[_\s]/i.test(header) ||
      /(number|no|num|code)$/i.test(headerLower) && cardinalityRatio > 0.8;
    ...
    const isFkByName = /(_id|_ref|_code|_num|_number|_key|Id|ID|Ref|Code)$/i.test(header) ||
      /^(fk_|ref_)/i.test(header) ||
      /(customer|account|branch|product|employee|order|user|company|department|category|type)(_?id|_?ref|_?code|_?num|_?number|Id|ID|Ref|Code)?$/i.test(headerLower);
    ...
    if (cardinalityRatio > 0.9 && values.length > 5) {
      // Also check if name hints at being an identifier
      const looksLikeId = /id|key|code|number|num|no|pk|uuid|guid/i.test(headerLower);
      if (looksLikeId) {
        return { type: 'id', xsdType: 'xsd:string', isId: true, isFkCandidate: false, isCategory: false };
      }
    }
