...
app.get('/api/health', async (req, res) => {
  ...
  try {
    ...
    // Check Trino
    try {
      const { defaultClient: trinoClient } = require('./config/trino');
...
async function startServer() {
  ...
  server = app.listen(PORT, async () => {
    ...
    try {
      ...
      // Start in-process BullMQ commit worker so jobs are processed
      // even when the separate worker process (server/workers/index.js) isn't running
      try {
        const { getProcessor: getCommitProcessor } = require('./services/jobProcessors/commitProcessor');
        getCommitProcessor();
        console.log('\n✅ In-process commit worker started');
      } catch (workerErr) {
        console.warn('⚠️  Could not start commit worker:', workerErr.message);
      }
