...
function OntologyJobs() {
  ...
  // Retry failed embeddings or re-embed from scratch for a committed document
  const retryEmbeddings = async (job) => {
    const docId = job.staged_doc_id || job.doc_id || job.document_id;
    if (!docId) {
      alert('Cannot determine document ID for this job');
      return;
    }
    try {
      // First try retry-embeddings (uses saved pending chunks)
      const response = await fetch(`${API_BASE_URL}/ontology/documents/${encodeURIComponent(docId)}/retry-embeddings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...getTenantHeaders() },
        body: JSON.stringify({
      ...
      if (data.success && data.total > 0) {
        alert(`🔄 Retrying ${data.total} embeddings in background...`);
        setTimeout(loadJobs, 3000);
        return;
      }
      
      // No pending data — fall back to re-embed from scratch
      if (data.retried === 0 || data.total === 0 || data.usePending) {
        const reembedRes = await fetch(`${API_BASE_URL}/ontology/documents/${encodeURIComponent(docId)}/re-embed`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...getTenantHeaders() },
          body: JSON.stringify({
            tenantId: currentWorkspace?.tenant_id || 'default',
        ...
        if (reembedData.success) {
          alert(`🔄 ${reembedData.message}`);
          setTimeout(loadJobs, 3000);
        } else {
          alert(`❌ ${reembedData.error || 'Re-embed failed'}`);
        }
        return;
      }
      
      alert(`🔄 ${data.message || 'Embeddings processed'}`);
      setTimeout(loadJobs, 3000);
    } catch (error) {
      console.error('Error retrying embeddings:', error);
      alert(`❌ ${error.message}`);
    }
  };
          ...
          {jobs.map(job => {
            ...
            return (
              ...
              >
                ...
                {/* Processing Steps - show for active and committed jobs */}
                {(isActive || job.status === 'committed') && job.processing_steps && (() => {
                  try {
                    const steps = typeof job.processing_steps === 'string' ? JSON.parse(job.processing_steps) : job.processing_steps;
                    if (!Array.isArray(steps)) return null;
                    // For committed jobs, only show if there's an embedding failure
                    if (job.status === 'committed' && !steps.some(s => s.failed > 0)) return null;
                    return (
                      <div className="oj-processing-steps">
                        {steps.map((step, idx) => (
                          <div 
                            key={step.step} 
                            className={`processing-step ${step.status} ${step.failed > 0 ? 'has-warning' : ''}`}
                          ...
                          >
                            ...
                            {step.status === 'completed' && step.failed > 0 && (
                              <span className="step-detail step-warning">⚠️ {step.failed} failed</span>
                            )}
                          </div>
                        ))}
                      </div>
                ...
                <div className="oj-item-actions" onClick={(e) => e.stopPropagation()}>
                  ...
                  {/* Committed jobs with embedding failures: Retry embeddings */}
                  {job.status === 'committed' && parseInt(job.embedding_failures) > 0 && (
                    <button onClick={() => retryEmbeddings(job)} className="redo" title={`Retry ${job.embedding_failures} failed embeddings`}>🔄 Retry Embeddings</button>
                  )}
                  {/* Committed jobs: Re-embed option (regenerate all embeddings from raw file) */}
                  {job.status === 'committed' && !parseInt(job.embedding_failures) && (
                    <button onClick={() => retryEmbeddings(job)} className="redo" title="Re-generate embeddings for RAG">🔄 Re-embed</button>
                  )}
      ...
      {selectedJob && (
        <>
          ...
          <div className="oj-details-content">
            ...
            {selectedJob.processing_steps && (() => {
              let steps = [];
              try {
                steps = typeof selectedJob.processing_steps === 'string' 
                  ? JSON.parse(selectedJob.processing_steps) 
                  : selectedJob.processing_steps;
              } catch { steps = []; }
              ...
              <div className="detail-section">
                ...
                <div className="processing-timeline">
                  {steps.map((step, idx) => (
                    <div key={step.step} className={`timeline-step ${step.status} ${step.failed > 0 ? 'has-warning' : ''}`}>
                      ...
                      <div className="timeline-content">
                        ...
                        <div className="timeline-details">
                          ...
                          {step.stored !== undefined && <span>{step.stored} stored</span>}
                          {step.failed > 0 && <span className="step-warning">⚠️ {step.failed} failed</span>}
            ...
            {selectedJob.status === 'committed' && (
              <div className="detail-section commit-summary">
                ...
                {parseInt(selectedJob.embedding_failures) > 0 && (
                  <div className="commit-embed-warning" style={{ marginTop: '8px', padding: '8px 12px', background: '#fff3cd', borderRadius: '6px', color: '#856404', fontSize: '13px' }}>
                    ⚠️ {selectedJob.embedding_failures} chunk(s) failed embedding — RAG search may be incomplete.
                    <button 
                      onClick={() => retryEmbeddings(selectedJob)} 
                      style={{ marginLeft: 8, padding: '2px 10px', borderRadius: 4, border: '1px solid #856404', background: '#fff', cursor: 'pointer', fontSize: 12 }}
                    >
                      🔄 Retry Embeddings
                    </button>
                  </div>
                )}
                {parseInt(selectedJob.embedding_failures) === 0 && selectedJob.processing_steps && (() => {
                  try {
                    const steps = typeof selectedJob.processing_steps === 'string' ? JSON.parse(selectedJob.processing_steps) : selectedJob.processing_steps;
                    const embedStep = steps?.find(s => s.step === 'embedding');
                    if (embedStep?.stored > 0) {
                      return (
                        <div style={{ marginTop: '8px', padding: '8px 12px', background: '#d1fae5', borderRadius: '6px', color: '#065f46', fontSize: '13px' }}>
                          ✅ {embedStep.stored} chunk(s) embedded for RAG search
                        </div>
                      );
                    }
                    if (embedStep?.chunks === 0) {
                      return (
                        <div style={{ marginTop: '8px', padding: '8px 12px', background: '#fef3c7', borderRadius: '6px', color: '#92400e', fontSize: '13px' }}>
                          ⚠️ No chunks were available for embedding — RAG search won't work for this document
                        </div>
                      );
                    }
                    return null;
                        ...
                        {selectedJob.suggested_ontology.entities.map((e, i) => (
                          <div key={i} className="entity-preview-item entity-with-props">
                            <div className="entity-header">
                              ...
                              <span className="entity-class-badge">{e.type}</span>
                              {e.confidence != null && (
                                <span className={`entity-confidence-badge ${e.confidence >= 0.85 ? 'high' : e.confidence >= 0.65 ? 'medium' : 'low'}`}
                                      title={`Extraction confidence: ${Math.round(e.confidence * 100)}%`}>
                                  {e.confidence >= 0.85 ? 'High' : e.confidence >= 0.65 ? 'Medium' : 'Low'} ({Math.round(e.confidence * 100)}%)
                                </span>
                              )}
                              {e.claimStatus && (
                                <span className={`entity-claim-status ${e.claimStatus.toLowerCase()}`}>
                                  {e.claimStatus === 'FACT' ? 'Verified' : e.claimStatus === 'CLAIM' ? 'Unverified' : e.claimStatus}
                                </span>
                              )}
                            </div>
                            {e.properties && Object.keys(e.properties).filter(k => e.properties[k] != null).length > 0 && (
                              <div className="entity-attributes-grid">
                                {Object.entries(e.properties).filter(([_, v]) => v != null).map(([key, value]) => (
                                  <div key={key} className="entity-attr-item">
                                    <span className="entity-attr-key">{key.replace(/_/g, ' ')}</span>
                                    <span className="entity-attr-value">{String(value)}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                            {e.sourceSpan && (
                              <div className="entity-evidence-quote">
                                <span className="evidence-quote-text">"{e.sourceSpan.substring(0, 150)}{e.sourceSpan.length > 150 ? '...' : ''}"</span>
                                {e.page && <span className="evidence-page-badge">Page {e.page}</span>}
                              </div>
                            )}
                          </div>
                        ))}
                        ...
                        {selectedJob.suggested_ontology.relationships.map((r, i) => (
                          <div key={i} className="rel-preview-item rel-with-props">
                            <div className="rel-header">
                              <span className="rel-source-entity">
                                <span className="rel-entity-class">{r.sourceType || ''}</span>
                                {r.sourceLabel || r.source || '?'}
                              </span>
                              <span className="rel-predicate-badge">
                                {(r.predicate || r.type || 'RELATED_TO').replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}
                              </span>
                              <span className="rel-arrow">→</span>
                              <span className="rel-target-entity">
                                <span className="rel-entity-class">{r.targetType || ''}</span>
                                {r.targetLabel || r.target || '?'}
                              </span>
                              {r.confidence != null && (
                                <span className={`rel-confidence-badge ${r.confidence >= 0.85 ? 'high' : r.confidence >= 0.65 ? 'medium' : 'low'}`}
                                      title={`Extraction confidence: ${Math.round(r.confidence * 100)}%`}>
                                  {r.confidence >= 0.85 ? 'High' : r.confidence >= 0.65 ? 'Medium' : 'Low'} ({Math.round(r.confidence * 100)}%)
                                </span>
                              )}
                            </div>
                            {r.properties && Object.keys(r.properties).filter(k => r.properties[k] != null).length > 0 && (
                              <div className="rel-attributes-grid">
                                {Object.entries(r.properties).filter(([_, v]) => v != null).map(([key, value]) => (
                                  <div key={key} className="rel-attr-item">
                                    <span className="rel-attr-key">{key.replace(/_/g, ' ')}</span>
                                    <span className="rel-attr-value">{String(value)}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                            {r.sourceSpan && (
                              <div className="rel-evidence-quote">
                                <span className="evidence-quote-text">"{r.sourceSpan.substring(0, 150)}{r.sourceSpan.length > 150 ? '...' : ''}"</span>
                                {r.page && <span className="evidence-page-badge">Page {r.page}</span>}
                              </div>
                            )}
                          </div>
                        ))}
          ...
          <div className="oj-details-actions">
            ...
            {selectedJob.status === 'committed' && (
              <>
                {parseInt(selectedJob.embedding_failures) > 0 ? (
                  <button onClick={() => { retryEmbeddings(selectedJob); }} className="btn primary">
                    🔄 Retry Embeddings ({selectedJob.embedding_failures} chunks)
                  </button>
                ) : (
                  <button onClick={() => { retryEmbeddings(selectedJob); }} className="btn secondary">
                    🔄 Re-embed for RAG
                  </button>
                )}
                {canDelete && (
