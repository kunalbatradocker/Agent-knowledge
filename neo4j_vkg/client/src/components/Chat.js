        ...
        {messages.map((msg, index) => (
          <div key={index} className={`message ${msg.role}`}>
            ...
            <div className="message-content">
                ...
                <div className="vkg-evidence-panels">
                  ...
                  {msg.contextGraph && msg.contextGraph.statistics?.nodeCount > 0 && (
                    <details className="vkg-context-graph-summary">
                      <summary>🕸️ Evidence Graph — {msg.contextGraph.statistics.nodeCount} entities, {msg.contextGraph.statistics.edgeCount} relationships</summary>
                      <div className="vkg-graph-details">
                        {msg.contextGraph.statistics.cardinality && (
                          <div className="vkg-cardinality">
                            {Object.entries(msg.contextGraph.statistics.cardinality).map(([type, count]) => (
                              <span key={type} className="vkg-entity-badge">{type}: {count}</span>
                            ))}
                          </div>
                        )}
                        {msg.contextGraph.nodes?.slice(0, 10).map((node, i) => (
                          <div key={i} className="vkg-node-item">
                            <span className="vkg-node-type">{node.type}</span>
                            <span className="vkg-node-value">{node.value}</span>
                          </div>
                        ))}
                        {msg.contextGraph.nodes?.length > 10 && (
                          <p className="vkg-more">...and {msg.contextGraph.nodes.length - 10} more entities</p>
                        )}
                      </div>
                    </details>
                  )}
                  ...
                  {msg.reasoningTrace?.length > 0 && (
                    <details className="vkg-reasoning-trace">
                      <summary>🧠 Reasoning Trace ({msg.reasoningTrace.length} steps)</summary>
                      <div className="vkg-trace-steps">
                        {msg.reasoningTrace.map((step, i) => (
                          <div key={i} className="vkg-trace-step">
                            <span className="vkg-step-num">{i + 1}</span>
                            <div className="vkg-step-content">
                              <p className="vkg-step-text">{step.step}</p>
                              {step.sources?.length > 0 && (
                                <span className="vkg-step-sources">Sources: {step.sources.join(', ')}</span>
                              )}
                            </div>
                          </div>
                        ))}
                  ...
                  {/* Execution Stats */}
                  {msg.executionStats && (
                    <div className="vkg-exec-stats">
                      <span>⏱️ {msg.executionStats.total_ms}ms</span>
                      <span>📊 {msg.executionStats.rows_returned} rows</span>
                      <span>🗄️ {msg.executionStats.databases_queried} DB{msg.executionStats.databases_queried !== 1 ? 's' : ''}</span>
                    </div>
                  )}

                  {/* Execution Pipeline */}
                  {msg.executionPipeline?.steps?.length > 0 && (
                    <details className="vkg-pipeline">
                      <summary>⚙️ Pipeline ({msg.executionPipeline.total_time_ms}ms total)</summary>
                      <div className="vkg-pipeline-steps">
                        {msg.executionPipeline.steps.map((step, i) => (
                          <div key={i} className={`vkg-pipeline-step ${step.status}`}>
                            <span className="vkg-pipe-name">{step.name}</span>
                            <span className="vkg-pipe-time">{step.duration_ms}ms</span>
                            <span className={`vkg-pipe-status ${step.status}`}>{step.status === 'success' ? '✓' : '✗'}</span>
                          </div>
                        ))}
                      </div>
                    </details>
              ...
              {msg.sources && !Array.isArray(msg.sources) && msg.searchMode !== 'vkg' && (
