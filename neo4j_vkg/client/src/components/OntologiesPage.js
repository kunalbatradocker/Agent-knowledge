...
const OntologiesPage = () => {
  ...
  const [showVersionHistory, setShowVersionHistory] = useState(false); // legacy, kept for handleVersionRollback
  ...
  // Version history rollback handler
  const handleVersionRollback = async () => {
    await fetchOntologies();
    if (selectedOntology && selectedOntology.id) {
      try {
        const response = await fetch(`${API_BASE_URL}/ontology/custom-ontology/${selectedOntology.id}`);
        if (response.ok) {
          const data = await response.json();
        ...
        console.error('Error refreshing ontology after rollback:', error);
      }
    }
  ...
  return (
    <div className="ontologies-page">
      ...
      {showVersionHistory && selectedOntology && (
        <VersionHistoryModal 
          ontology={selectedOntology} 
