...
- The primary key column of THIS sheet (e.g. the ID column) → includeAsNode=false, type="id"`;
  ...
  async analyzeForMapping(headers, sampleRows, ontology, options = {}) {
    ...
    if (sheets && sheets.length > 1) {
      ...
      return { primaryClass, primaryClassExplanation: `Per-sheet analysis`, mappings: dedupedMappings, sheetPrimaryClasses };
    }
...
- Data properties MUST have primaryClass as their domain (or be marked as new)
- Object properties MUST have primaryClass as their domain
- If a property exists but has wrong domain, mark propertyIsNew=true and suggest new name
- Date/time columns → ALWAYS isLiteral=true
- Numeric columns → ALWAYS isLiteral=true  
- Foreign key columns → isLiteral=false, linkedClass = target entity
- Prefer EXISTING properties/classes over creating new ones
- Use semantic property names: "hasCustomer" not "customer_id"`;
