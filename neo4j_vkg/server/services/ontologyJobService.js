...
class OntologyJobService {
  ...
  async getJob(jobId) {
    ...
    if (hasLegacySteps || (isTerminal && steps.some(s => s.status === 'active'))) {
      ...
      if (correctSteps[jobType]) {
        // Mark failed steps if job failed
        const fixed = correctSteps[jobType].map(s => 
          data.status === 'failed' && s.status === 'pending' ? { ...s, status: 'skipped' } : s
        );
