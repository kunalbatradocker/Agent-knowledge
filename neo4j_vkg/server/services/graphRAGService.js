...
SELECT ?type (COUNT(?entity) AS ?count) WHERE {
  ...
  async queryNeo4jDirect(question, options = {}) {
    ...
    try {
      const cypher = await this.generateCypher(question, options.workspace_id);
...
Provide a clear, concise answer. If there are numbers, summarize them. If it's a list, format it nicely.`
  ...
  async queryGraphOnly(question, options = {}) {
    ...
    try {
      ...
      console.log('-'.repeat(40));
      const cypher = await this.generateGraphOnlyCypher(question, schemaText, graphDepth, options.workspace_id);
      console.log(`   Generated Cypher: ${cypher}`);
...
Provide a clear, concise answer. Format tables and lists nicely using markdown.`
   ...
   */
  async generateGraphOnlyCypher(question, schemaText, maxHops = 2, workspaceId = null) {
...
7. DO NOT hallucinate or guess full names - use exactly what's in the query`
   ...
   */
  async generateCypher(question, workspaceId = null) {
...
- ChangeEvent nodes track entity changes (created, updated, deleted)
- Properties: changeType, property, previousValue, newValue, changedAt, sourceDocument, entity
- Relationship: (entity)-[:CHANGED]->(ChangeEvent)
- Use ChangeEvent nodes for questions about history, changes, edits, audit trail, or "what changed"
