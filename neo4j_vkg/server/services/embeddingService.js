...
class EmbeddingService {
  ...
  async _bedrockEmbed(text) {
      ...
      if (rawToken) {
        ...
        const LLMService = require('./llmService').constructor || require('./llmService').__proto__?.constructor;
        let effectiveRegion = this.bedrockRegion;
        try {
          // Parse region from the base64-encoded presigned URL inside the token
      ...
      // Fallback: use AWS SDK credential chain
