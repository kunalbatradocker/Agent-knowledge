...
const CATALOG_PATH = process.env.TRINO_CATALOG_PATH || path.join(__dirname, '../../trino/catalog');
...
class TrinoCatalogService {
   ...
   */
  async registerCatalog(tenantId, config) {
    ...
    await client.hSet(this._redisKey(tenantId), catalogName, JSON.stringify(metadata));

    // Test connectivity (only if Trino is reachable)
    try {
      const trinoHealth = await trinoClient.checkConnection();
      if (trinoHealth.connected) {
        await this.testCatalog(tenantId, catalogName);
   ...
   * Remove a catalog
   */
  async removeCatalog(tenantId, catalogName) {
    // Resolve: check if it exists as-is in Redis, else try namespaced
    const catalogs = await this.listCatalogs(tenantId);
    ...
    // Remove from Redis
    await redisService.hDel(this._redisKey(tenantId), fullName);
   ...
   */
  async listCatalogs(tenantId) {
    const raw = await redisService.hGetAll(this._redisKey(tenantId));
    if (!raw) return [];
   ...
   */
  async testCatalog(tenantId, catalogName) {
    ...
    try {
      ...
      const raw = await redisService.hGet(this._redisKey(tenantId), fullName);
      if (raw) {
        try {
          const m = JSON.parse(raw);
          m.status = 'active';
          delete m.error;
          await redisService.hSet(this._redisKey(tenantId), fullName, JSON.stringify(m));
   ...
   */
  async introspectCatalog(tenantId, catalogName, _preloadedCatalogs = null) {
      // Use preloaded catalog list if provided (avoids redundant Redis calls)
      const catalogs = _preloadedCatalogs || await this.listCatalogs(tenantId);
      let meta = catalogs.find(c => c.catalogName === catalogName);
   ...
   */
  async introspectAllCatalogs(tenantId) {
      ...
      const results = await Promise.all(
        eligibleCatalogs.map(async (cat) => {
          try {
            return await this.introspectCatalog(tenantId, cat.catalogName, catalogs);
          } catch (err) {
   ...
   */
  async discoverCatalogs(tenantId) {
    const health = await trinoClient.checkConnection();
    if (!health.connected) {
    ...
    const existing = await this.listCatalogs(tenantId);
    ...
    for (const catName of trinoCatalogs) {
      ...
      await redisService.hSet(this._redisKey(tenantId), catName, JSON.stringify(metadata));
