      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            if (status === 403) {
              console.error(`❌ [Bedrock] 403 response body: ${respBody}`);
              console.error(`❌ [Bedrock] URL: ${url}`);
              console.error(`❌ [Bedrock] Token length: ${bearerToken.length}, prefix present: ${bearerToken.startsWith('bedrock-api-key-')}`);
