...
class OWLOntologyService {
  ...
  async createVersion(tenantId, workspaceId, ontologyId, versionData) {
    try {
      const versionRecord = {
        id: `${ontologyId}_v${versionData.version}`,
        ontologyId,
        version: versionData.version,
        created: new Date().toISOString(),
        description: versionData.description || '',
        author: versionData.author || 'System',
        sourceGlobal: versionData.sourceGlobal || null
      };

      // Store in GraphDB as metadata
      const versionGraphIRI = `http://purplefabric.ai/graphs/tenant/${tenantId}/workspace/${workspaceId}/versions/${ontologyId}`;
      
      const versionTurtle = `
        @prefix owl: <http://www.w3.org/2002/07/owl#> .
        @prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
        @prefix version: <http://purplefabric.ai/version#> .
        
        <${versionRecord.id}> a version:OntologyVersion ;
          version:ontologyId "${ontologyId}" ;
          version:version "${versionRecord.version}" ;
          version:created "${versionRecord.created}" ;
          version:description "${versionRecord.description}" ;
          version:author "${versionRecord.author}" .
      `;

      await graphDBStore.importTurtle(tenantId, workspaceId, versionTurtle, null, 'data', `versions_${ontologyId}`);
      
      return versionRecord;

    } catch (error) {
      logger.error(`Failed to create version for ${ontologyId}:`, error);
      throw error;
    }
  }
  ...
  async getVersionHistory(tenantId, workspaceId, ontologyId) {
    try {
      ...
      const query = `
        ...
        SELECT ?id ?version ?created ?description ?author
        FROM <${versionGraphIRI}>
        WHERE {
          ?id a version:OntologyVersion ;
              version:ontologyId "${ontologyId}" ;
      ...
      const result = await graphDBStore.executeSPARQL(tenantId, workspaceId, query);
      ...
      logger.error(`Failed to get version history for ${ontologyId}:`, error);
      return [];
    }
  }

  /**
   * Save new version of ontology
   */
  async saveNewVersion(tenantId, workspaceId, ontologyId, turtleContent, versionInfo) {
    try {
      ...
      // Create version record
      await this.createVersion(tenantId, workspaceId, ontologyId, versionInfo);
    ...
    try {
      const query = `
        ...
        WHERE {
          ...
          FILTER(STRSTARTS(STR(?g), "http://purplefabric.ai/graphs/tenant/${tenantId}/workspace/${workspaceId}/versions"))
        }
        ORDER BY DESC(?created)
      `;
