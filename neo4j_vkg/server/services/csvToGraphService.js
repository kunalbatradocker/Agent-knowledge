...
class CsvToGraphService {
  ...
  generateNodeUri(type, value) {
    ...
    return `csv://${type.toLowerCase()}/${normalizedValue}`;
  }
  ...
  convertWithOntology(parsedCSV, ontology, columnMapping = {}, relationshipMapping = []) {
    ...
    // Build column to ontology class mapping
    const effectiveMapping = {};
    headers.forEach(header => {
      if (columnMapping[header]) {
        effectiveMapping[header] = columnMapping[header];
      } else {
        // Try to auto-match column to ontology class
        const normalizedHeader = header.toLowerCase().replace(/[_\s-]+/g, '');
        const matchedClass = Array.from(ontologyClasses).find(cls => 
          cls.replace(/[_\s-]+/g, '') === normalizedHeader ||
          normalizedHeader.includes(cls) ||
          cls.includes(normalizedHeader)
        );
        if (matchedClass) {
          effectiveMapping[header] = this.sanitizeTypeName(matchedClass);
        }
      }
    });
