...
router.get('/graphdb/stats', async (req, res) => {
  try {
    // Query GraphDB directly to get unscoped counts
    ...
    const [countRes, graphsRes] = await Promise.all([
      runQuery(`SELECT (COUNT(*) as ?count) WHERE { ?s ?p ?o }`),
      runQuery(`SELECT DISTINCT ?g (COUNT(*) as ?triples) WHERE { GRAPH ?g { ?s ?p ?o } } GROUP BY ?g`)
    ]);
    
    const totalTriples = parseInt(countRes?.results?.bindings?.[0]?.count?.value || 0);
    const graphs = graphsRes?.results?.bindings || [];
    
    const ontologyGraphs = graphs.filter(g => g.g?.value?.includes('/ontology/'));
    const dataGraphs = graphs.filter(g => g.g?.value?.includes('/data'));
...
router.post('/graphdb/cleanup', async (req, res) => {
  try {
    ...
    const clearGraphByIRI = async (graphIRI) => {
      const url = `${process.env.GRAPHDB_URL || 'http://localhost:7200'}/repositories/${process.env.GRAPHDB_REPOSITORY || 'knowledge_graph_1'}/statements`;
      const response = await fetch(url, {
        method: 'POST',
    ...
    if (type === 'data') {
      ...
      for (const b of result?.results?.bindings || []) {
        const g = b.g?.value;
        // Skip global ontology graphs
        if (g && !g.includes('/graphs/global/')) {
      ...
      message = `Cleared ${cleared} graphs (global ontologies preserved)`;
    }
...
router.get('/redis/stats', async (req, res) => {
  try {
    ...
    // Use vectorStoreService for accurate stats (covers both old and new format)
    const vsStats = await vectorStoreService.getStats();
    
    // Count conversation keys separately
    let convCount = 0;
    let cursor = '0';
    do {
    ...
    res.json({
      ...
      totalKeys,
      memoryUsed: memMatch ? memMatch[1] : '—',
...
router.post('/redis/cleanup', async (req, res) => {
  try {
    ...
    // "Clear All" only removes user-generated content: vectors, chunks, conversations, raw docs
    // It does NOT touch: auth keys, tenant/workspace config, ontology config, settings
    ...
    if (type === 'all') {
      // Delete raw doc data and workspace doc indexes, staged data, extraction data
...
router.get('/redis/stats', async (req, res) => {
  try {
    ...
    if (!client.isOpen) {
      return res.json({ error: 'Redis not connected', stats: {} });
    }
    
    const patterns = [
      'tenant:*', 'workspace:*', 'folder:*',
      'ontology_job:*', 'staged:*', 'doc:*', 
      'chunk:*', 'embedding:*', 'staged-extraction:*',
      'extraction:*', 'doc_extraction:*', 'concept_*',
      'schema:*', 'ontology:*'
    ];
 ...
 * Clean up orphaned/legacy Redis keys
 */
router.post('/redis/cleanup', async (req, res) => {
  try {
    const { client } = require('../config/redis');
    ...
    // Default patterns to clean if none specified
    const toClean = patterns.length > 0 ? patterns : [
      'concept_refs:*',  // Legacy concept tracking
      'concept_meta:*'   // Legacy concept metadata
    ];
    ...
    for (const pattern of toClean) {
      ...
      do {
        ...
        if (result[1].length > 0) {
          ...
          patternDeleted += result[1].length;
        }
      } while (cursor !== '0');
      if (patternDeleted > 0) {
        deleted[pattern] = patternDeleted;
      }
    }
    
    res.json({ 
      success: true, 
      message: `Cleaned ${Object.values(deleted).reduce((a, b) => a + b, 0)} keys`,
      deleted 
    });
  } catch (error) {
    logger.error('Redis cleanup error:', error);
